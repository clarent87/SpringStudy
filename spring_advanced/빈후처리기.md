...
- 중요 포인트 👍
  - 포인트컷이 아래 두가지 활용처에서 사용됨 (앞서 한번 언급했음)
    - AnnotationAwareAspectJAutoProxyCreator(자동 프록시 생성기)가 빈을 프록시로 만들지 말지 결정할때 한번 쓰고
    - 프록시를 만들어서 등록했다면, 해당 프록시의 method가 호출될때 
    - 해당 mehtod에 advice를 적용해야 될지 말지에서 한번 더 쓰인다. 
