...
public class ValidationItemControllerV2 {
    ...
    // 이 컨트롤러가 호출될 때마다 아래가 호출됨
    @InitBinder
    public void init(WebDataBinder dataBinder) {
        // dataBinder가 요청마다 새로 만들어 -> mvc 내부에서 항상 만드나봄 요청 때마다..
...
//    @PostMapping("/add")
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (!StringUtils.hasText(item.getItemName())) {
            // bindingFailure는 바인딩에러 난 케이스에서만 true를 써주는것.
...
//    @PostMapping("/add")
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (!StringUtils.hasText(item.getItemName())) {
            ...
            // errorCode의 규칙 중요. vscode에 언급했었음 (요구사항.객체.필드 형태)
...
//    @PostMapping("/add")
    public String addItemV5(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // itemValidator.suppert를 이용해서 item이 내가 검증하려는 class가 맞는지 체크 해도된다.
